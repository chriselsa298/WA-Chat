 ...
 *
 * Auth: shared with `/api/automations/cron` so operators only have one
 * secret to provision — see `@/lib/cron/auth` for the accepted headers.
 * The two endpoints are independent operations; we keep them on
 * separate URLs so one failing doesn't block the other.
 *
